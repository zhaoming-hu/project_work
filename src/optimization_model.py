...
from constraints import V2GConstraintsCase2
...
class V2GOptimizationModelCase1:
    ...
    def build_model(self):
        ...
        # 定义主决策变量 - 场景无关的投标决策变量
        P_ev0_total = self.model.addVars(self.T, name="P_ev0_total") # 能源投标 - 场景无关
        R_ev_up = self.model.addVars(self.T, name="R_ev_up") # 上调频容量投标 - 场景无关 
        R_ev_dn = self.model.addVars(self.T, name="R_ev_dn") # 下调频容量投标 - 场景无关
        ...
        P_es_ch_i = self.model.addVars(keys_cc, lb=0, name="P_es_ch_i") # ES2 charging power allocated to controllable EV i in hour t in scenario w (MW)
        P_es_dis_i = self.model.addVars(keys_cc, lb=0, name="P_es_dis_i") # ES2 discharging power allocated to controllable EV i in hour t in scenario w (MW)
        ...
        for w in range(self.num_scenarios):
            ...
            for t in range(self.T):
                ...
                self.model.addConstr(
                    F_ev_mil[w, t] == (R_ev_up[t] + R_ev_dn[t]) * price_scenario['balancing_prices'].iloc[t] * self.delta_t
                )
...
class V2GOptimizationModelCase2:
    ...
    def build_model(self):
        self.model = gp.Model("V2G_Optimization_Case2")
        constraints = V2GConstraintsCase2(self.model)
        ...
        # 定义主决策变量 - 场景无关变量
        P_ev0_total = self.model.addVars(self.T, name="P_ev0_total") # 能源投标 - 场景无关
        R_ev_up = self.model.addVars(self.T, name="R_ev_up") # ev上调频容量投标 - 场景无关 
        R_ev_dn = self.model.addVars(self.T, name="R_ev_dn") # ev下调频容量投标 - 场景无关
        P_es0 = self.model.addVars(self.T, name="P_es0") # Energy bids (also PSP) of ES in hour t (MW)
        ...
        E_es1_init  = self.model.addVar(name="E_es1_init") # Initial energy stored in ES1 (MW)
        E_es2_init = self.model.addVar(name="E_es2_init") # Initial energy stored in ES2 (MW)
        ...
        P_es2_ch_i = self.model.addVars(keys_cc, lb=0, name="P_es2_ch_i") # ES2 charging power allocated to controllable EV i in hour t in scenario w (MW)
        P_es2_dis_i = self.model.addVars(keys_cc, lb=0, name="P_es2_dis_i") # ES2 discharging power allocated to controllable EV i in hour t in scenario w (MW)
        ...
        for w in range(self.num_scenarios):
            ...
            for t in range(self.T):
                ...
                # 储能系统退化成本（支出项，需从总收益中扣除）
                degradation_factor = 72700  # 固定值替代 2 * L * d * eta_ch * eta_dis
                self.model.addConstr(
                    F_es_deg[w, t] == self.C_es * (P_es_ch[w, t] + P_es_dis[w, t]) / degradation_factor
                )

                expr += (F_ev_charging[w, t] + F_ev_cap[w, t] + F_ev_mil[w, t] - F_ev_deploy[w, t] +
                         F_es_arb[w, t]- F_es_deg[w, t])
            self.model.addConstr(f_w[w] == expr)
            scenario_revenues.append(f_w[w])  # 添加到场景收益列表
...
class V2GOptimizationModelCase3:
    ...
    def build_model(self):
        ...
        R_ev_up = self.model.addVars(self.T, name="R_ev_up") # 上调频容量投标 - 场景无关 
        R_ev_dn = self.model.addVars(self.T, name="R_ev_dn") # 下调频容量投标 - 场景无关
        R_es_up = self.model.addVars(self.T, name="R_es_up") # Regulation up capacity bids of ES in hour t (MW)
        R_es_dn = self.model.addVars(self.T, name="R_es_dn") # Regulation down capacity bids of ES in hour t (MW)
        P_es0 = self.model.addVars(self.T, name="P_es0") # Energy bids (also PSP) of ES in hour t (MW)
        E_es1_max = self.model.addVar(name="E_es1_max") # Maximum energy stored in ES1 (MW)
        E_es2_max = self.model.addVar(name="E_es2_max") # Maximum energy stored in ES2 (MW)
        E_es1_init  = self.model.addVar(name="E_es1_init") # Initial energy stored in ES1 (MW)
...
class V2GOptimizationModelCase4:
    def __init__(
        self,
        reduced_ev_scenarios: List[pd.DataFrame],  #传入ev的数据
        reduced_price_scenarios: List[pd.DataFrame],  #传入价格数据：dam rtm价格和投标价、激活价
    ...
    def build_model(self):
        self.model = gp.Model("V2G_Optimization_Case4")
        constraints = V2GConstraintsCase4(self.model)
